...
def build_svg(langs_size: dict[str, int], langs_repos: dict[str, list[tuple[str, int]]], top_n: int = 6) -> str:
    ...
    FOOTER_H = 44
    SEP_H = 20  # vertical gap per block (includes separator)
    # Calculate dynamic height
    H = TITLE_H + PAD
    for lang, _ in ranked:
        repos = langs_repos.get(lang, [])
        num_shown = min(len(repos), 2)
        block_h = 34 + num_shown * 14 + SEP_H
        H += block_h
    H += FOOTER_H
...
<line x1="{PAD}" y1="36" x2="{W - PAD}" y2="36" stroke="{GRV["bg2"]}"/>"""
    ...
    for i, (lang, count) in enumerate(ranked):
        ...
        byte_str = format_bytes(count)
        # Language header row
        out.append(f"""
<circle cx="{PAD + 5}" cy="{y + 10}" r="4" fill="{color}"/>
<text x="{PAD + 16}" y="{y + 14}" font-family={FONT!r} font-size="12.5" font-weight="600" fill="{GRV["fg"]}">{lang}</text>
<text x="{W - PAD - 70}" y="{y + 14}" font-family={FONT!r} font-size="10" fill="{GRV["gray"]}" text-anchor="end">{byte_str}</text>
<text x="{W - PAD}" y="{y + 14}" font-family={FONT!r} font-size="10.5" fill="{GRV["dimgray"]}" text-anchor="end">{pct * 100:.1f}%</text>
...
<rect x="{PAD}" y="{y + 20}" width="{bar_fill}" height="{BAR_H}" rx="2.5" fill="{color}" opacity="0.85"/>""")
        ...
        repo_start = y + 34
        for r_idx, (repo_name, _) in enumerate(repos[:2]):
            out.append(f"""
<text x="{PAD + 16}" y="{repo_start + r_idx * 14}" font-family={FONT!r} font-size="9.5" fill="{GRV["dimgray"]}">└─ {repo_name}</text>""")
        if len(repos) > 2:
            more = len(repos) - 2
            out.append(f"""
<rect x="{PAD + 16}" y="{repo_start + 2 * 14 - 7}" width="22" height="14" rx="3" fill="{GRV["bg1"]}"/>
<text x="{PAD + 27}" y="{repo_start + 2 * 14}" font-family={FONT!r} font-size="9" font-weight="600" fill="{GRV["gray"]}" text-anchor="middle">+{more}</text>""")
        # Separator line
        sep_y = repo_start + min(len(repos), 2) * 14 + 10
        out.append(f"""
<line x1="{PAD}" y1="{sep_y}" x2="{W - PAD}" y2="{sep_y}" stroke="{GRV["bg1"]}" stroke-dasharray="3 4"/>""")
        num_repo_lines = min(len(repos), 2)
        y += 34 + num_repo_lines * 14 + SEP_H
    # Footer
    footer_y = H - FOOTER_H + 14
    total_repos = sum(len(r) for r in langs_repos.values())
    out.append(f"""
<line x1="{PAD}" y1="{footer_y - 6}" x2="{W - PAD}" y2="{footer_y - 6}" stroke="{GRV["bg1"]}"/>
<text x="{PAD}" y="{footer_y + 6}" font-family={FONT!r} font-size="9" fill="{GRV["dimgray"]}">∑ {total_repos} repos · updated {now}</text>
<text x="{W - PAD}" y="{footer_y + 6}" font-family={FONT!r} font-size="9" fill="{GRV["dimgray"]}" text-anchor="end">github.com/{USERNAME}</text>""")
    out.append("\n</svg>")
    return "".join(out)
if __name__ == "__main__":
    ...
    if svg:
        ...
        with open("assets/languages.svg", "w") as f:
            f.write(svg)
        print("✓ assets/languages.svg generated successfully")
    else:
